...
//    $console->add($command);
    ...
    $command = new Command(
        'process:full_rebuild',
        'PHPDocTool\Cli\FullRebuild::run'
    );
    $command->setDescription("Scan for modified files.");
    $console->add($command);
...
function addStatusCommands(Application $console)
{
    ...
    $command = new Command('status:get', 'PHPDocTool\Cli\Status::get');
    $command->setDescription("Get the overall status of the build system.");
    $console->add($command);
}
